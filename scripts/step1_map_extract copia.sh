...
# ── strip any stray quotes around those paths ─────────────────────────────
...
# 4) Taxonomy conversion (auto-detect header row in any sheet / CSV / TSV)
...
if ext in ('.xlsx','.xls'):
    ...
    if sheet != requested:
        sys.stderr.write("Warning: sheet '%s' not found; using '%s'\n" % (requested, sheet))
    raw = pd.read_excel(xlsx, sheet_name=sheet, header=None)
    header_row = None
    for idx, row in raw.iterrows():
        vals = row.astype(str).str.strip().tolist()
        if all(col in vals for col in wanted):
            header_row = idx
            break
    if header_row is None:
        raise ValueError("Could not find header row with columns %s" % wanted)
    ...
    raise ValueError("Unsupported taxonomy file extension: %s" % ext)

missing = [c for c in wanted if c not in df.columns]
if missing:
    raise ValueError("Missing columns in taxonomy data: %s" % missing)
